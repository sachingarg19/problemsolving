...
*/
public class DesignerPdfViewerProblem {

    // Complete the designerPdfViewer function below.
    static int designerPdfViewer(int[] h, String word) {
        System.out.println(word);
        String alphabets = "abcdefghijklmnopqrstuvwxyz";
        int wordLen= alphabets.length();
        System.out.println("h size " + h.length + " alpha size - " + wordLen);
        Map<Character, Integer> wordLenMap = new HashMap<>();
        
        for(int i=0; i < wordLen; i++){
            wordLenMap.put(alphabets.charAt(i), h[i]);
        }
        System.out.println(wordLenMap);
        int max = 0;
        for(int j=0; j < word.length(); j++){
            max = Math.max(max,wordLenMap.get(word.charAt(j)));
        }
